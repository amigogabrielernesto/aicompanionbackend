...
export async function askAIChat(message: string, context: string[]): Promise<AIChatResponse> {
    const ai = getAI();
...
`;
    ...
    try {
        ...
        const parsed = JSON.parse(text) as AIChatResponse;
        ...
        return {
            message:
                "Entiendo lo que me dices. Estoy aquí para escucharte y acompañarte en este momento. A veces hablarlo ayuda a poner las cosas en perspectiva.",
        };
    }
}